...
impl fmt::Display for ConfigurationError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ConfigurationError::EnvOverrideError(msg) => write!(f, "Environment override error: {}", msg),
            ConfigurationError::ValidationError(msg) => write!(f, "Configuration validation error: {}", msg),
            ConfigurationError::DecryptionError(msg) => write!(f, "Configuration decryption error: {}", msg),
...
impl From<serde_json::Error> for ConfigurationError {
    fn from(err: serde_json::Error) -> Self {
        ConfigurationError::ParseError(format!("{} at line {}, column {}", 
            err.to_string(), 
            err.line(), 
            err.column()))
    }
...
fn apply_env_overrides(config: AppConfig) -> Result<AppConfig, ConfigurationError> {
    ...
    let mut json_value = serde_json::to_value(&config)
        .map_err(|err| ConfigurationError::ParseError(format!("Failed to serialize config to JSON: {}", err)))?;
    ...
    let config_with_env_vars = serde_json::from_value(json_value)
        .map_err(|err| ConfigurationError::ParseError(format!("Failed to deserialize config with env vars: {}", err)))?;
...
fn decrypt_config(config: AppConfig) -> Result<AppConfig, ConfigurationError> {
    ...
    secrets::process_json_for_decryption(&mut json_value)
        .map_err(|err| ConfigurationError::DecryptionError(format!("Failed to decrypt config: {}", err)))?;

    // Convert back to AppConfig
    let decrypted_config = serde_json::from_value(json_value)
        .map_err(|err| ConfigurationError::ParseError(format!("Failed to deserialize decrypted config: {}", err)))?;
