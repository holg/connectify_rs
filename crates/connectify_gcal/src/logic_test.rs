...
mod tests {
        ...
        if is_working_day {
            ...
            for i in 1..slots.len() {
                ...
                assert!(time_diff >= duration, 
                    "Slots should be at least duration apart: {:?} and {:?}", slots[i-1], slots[i]);
            }
        } else {
            // If it's not a working day, we should have no slots
            assert!(slots.is_empty(), "Should have no available slots on a non-working day");
        }
    }
    ...
    fn test_calculate_available_slots_with_busy_periods() {
        ...
        if is_working_day {
            ...
            for slot in &slots {
                ...
                assert!(slot_end <= busy_start || *slot >= busy_end,
                    "Slot should not overlap with busy period: {:?} to {:?}", slot, slot_end);
            }
        } else {
            // If it's not a working day, we should have no slots
            assert!(slots.is_empty(), "Should have no available slots on a non-working day");
    ...
    fn test_calculate_available_slots_with_buffer() {
        ...
        if is_working_day {
            ...
            for i in 1..slots.len() {
                let time_diff = slots[i] - slots[i-1];
                assert!(time_diff >= duration + buffer, 
                    "Slots should be at least duration + buffer apart: {:?} and {:?}", slots[i-1], slots[i]);
            }
        } else {
            // If it's not a working day, we should have no slots
            assert!(slots.is_empty(), "Should have no available slots on a non-working day");
    ...
    fn test_calculate_available_slots_respects_working_hours() {
        ...
        if is_working_day {
            ...
            for slot in &slots {
                ...
                assert!(slot_time >= work_start, 
                    "Slot should start after work hours begin: {:?}", slot);
                assert!(slot_end_time <= work_end, 
