...
use actix_web::{
    get, web, HttpRequest, HttpResponse, Responder, Result as ActixResult, error, http::header
};
use oauth2::{
...
pub async fn calendly_auth_callback(
    ...
    let encrypted_access_token = crypto::encrypt(&config.calendly_config.encryption_key.to_vec(), access_token.secret().as_bytes())
    ...
    let encrypted_refresh_token = refresh_token_opt.map(|rt| {
        crypto::encrypt(&config.calendly_config.encryption_key.to_vec(), rt.secret().as_bytes())
...
pub fn print_calendly_oauth_url(config: &AppConfig) {
    if let (client_id, redirect_uri) = (&config.calendly_config.client_id, &config.calendly_config.redirect_uri) {
