...
mod tests {
    ...
    async fn test_create_and_get_events() {
        ...
        let events = service
            .get_booked_events(calendar_id, now, now + Duration::hours(3), false)
            .await
            .unwrap();
        ...
        let cancel_result = service
            .mark_event_cancelled(calendar_id, &event_id, true)
            .await
            .unwrap();
        ...
        let events = service
            .get_booked_events(calendar_id, now, now + Duration::hours(3), true)
            .await
            .unwrap();
        ...
        // Delete the event
        service
            .delete_event(calendar_id, &event_id, true)
            .await
            .unwrap();
    ...
    async fn test_busy_times() {
        ...
        // Get the busy times
        let busy_times = service
            .get_busy_times(calendar_id, now, now + Duration::hours(3))
            .await
            .unwrap();
        ...
        let result = service
            .create_event(calendar_id, overlapping_event.clone())
            .await;
        ...
        // This should succeed
        let result = service
            .create_event(calendar_id, non_overlapping_event.clone())
            .await;
        assert!(result.is_ok());

        // Get the busy times again
        let busy_times = service
            .get_busy_times(calendar_id, now, now + Duration::hours(5))
            .await
            .unwrap();
